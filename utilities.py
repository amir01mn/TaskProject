...
class SeaLife:
  ...
  def __init__(self, direction, pos):
    ...
    self.speed = (random.randint(SeaLife.speed_range[self.cat][0], SeaLife.speed_range[self.cat][1])) / 10
  ...
  def getStr(self):
    return int(self.pos) * " " + SeaLife.draw[self.cat][self.direction] + (
              (50 - int(self.pos) - 3 - 5) * " ") + self.cat
